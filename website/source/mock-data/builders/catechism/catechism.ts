...
import { getContainerDesignator } from '@catechism-utils/path-id.ts';
...
function setNaturalLanguagePathsHelper(content: ContentBase, ancestralPath: Array<string>, language: Language): void {
    ...
    const titleData = isTitleableContent(content) ? getContentTitleData(language, content) : null;
    if (titleData?.typeLabel) {
        title = titleData.typeLabel;
        if (titleData.contentNumber) {
            title += ' ' + titleData.contentNumber;
        }
    }
