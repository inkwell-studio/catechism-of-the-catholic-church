...
async function runTests(
    ...
    Deno.test(`[${languageKey}] the reference numbers for each Section and Chapter start at 1, and are contiguous after that`, () => {
        ...
        [...sections, ...chapters].forEach((sc) => {
            ...
            const referenceNumbers = textWrappers
                .map((tw) => tw.referenceCollection?.referenceNumber ?? null)
                .filter((refNum) => !!refNum);

            const firstReferenceNumber = referenceNumbers[0];
    ...
    Deno.test(`[${languageKey}] all references contain content`, () => {
        const content = getAllContent(catechism);
        const textWrappers = getAll<TextWrapper>(content, Content.TEXT_WRAPPER);
        const referenceCollectionsWithoutContent = textWrappers
            .map((tw) => tw.referenceCollection)
            .filter((rc) => !!rc)
            .filter((rc) => rc.references.length === 0);

        assert(
            referenceCollectionsWithoutContent.length === 0,
            `${referenceCollectionsWithoutContent.length} reference collections without content exist. Reference numbers: [${
                referenceCollectionsWithoutContent.map((rc) => rc.referenceNumber).join(', ')
            }]`,
        );
    });
